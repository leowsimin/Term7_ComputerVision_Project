...
from config import epoch_to_test, eval_mode, dataset, use_existing_model_weights, pck_metric
...
batch_size = 20
start, end = number_images - 2000, number_images
x_test = data[start:end]
y_test = [heatmap_set[start:end], coordinates[start:end], visibility[start:end]]
score_avg = model.evaluate(x=x_test, y=y_test, batch_size=batch_size, callbacks=[logger.keras_custom_callback])[-1]
print("AVG PCK SCORE:", score_avg)
