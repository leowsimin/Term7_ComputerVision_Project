...
class Deeper_Base_BlazePose():
    def __init__(self, l2_reg=0):
        ...
        self.conv_2_3 = tf.keras.models.Sequential([
            tf.keras.layers.DepthwiseConv2D(kernel_size=3,strides=2, padding='same', activation=None,   depthwise_regularizer=tf.keras.regularizers.L2(l2_reg)),
            tf.keras.layers.Conv2D(filters=48, kernel_size=1, activation=None,   kernel_regularizer=tf.keras.regularizers.L2(l2_reg))
        ])

        self.conv_2_4 = tf.keras.models.Sequential([
            tf.keras.layers.DepthwiseConv2D(kernel_size=3,strides=2, padding='same', activation=None,   depthwise_regularizer=tf.keras.regularizers.L2(l2_reg)),
            tf.keras.layers.Conv2D(filters=96, kernel_size=1, activation=None,   kernel_regularizer=tf.keras.regularizers.L2(l2_reg))
        ])

        self.conv_2_5 = tf.keras.models.Sequential([
            tf.keras.layers.DepthwiseConv2D(kernel_size=3,strides=2, padding='same', activation=None,   depthwise_regularizer=tf.keras.regularizers.L2(l2_reg)),
            tf.keras.layers.Conv2D(filters=192, kernel_size=1, activation=None,   kernel_regularizer=tf.keras.regularizers.L2(l2_reg))
        ])
    ...
    def call(self):
        ...
        # ---------- heatmap branch ----------
        # shape = (1, 128, 128, 24)
        y1 = self.conv3(y0) + x1 # output res: 64
        y2 = self.conv4(y1) + x2 # output res:  32
        y3 = self.conv5(y2) + x3 # output res:  16
        ...
        x = tf.keras.layers.Lambda(lambda t: tf.stop_gradient(t))(x)
        y2 = tf.keras.layers.Lambda(lambda t: tf.stop_gradient(t))(y2)
        y3 = tf.keras.layers.Lambda(lambda t: tf.stop_gradient(t))(y3)
        y4 = tf.keras.layers.Lambda(lambda t: tf.stop_gradient(t))(y4)
