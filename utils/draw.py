...
def draw_images_separate_joint(model, img_idxs: list[int]):  
    ...
    for t in img_idxs:
        fig = plt.figure(figsize=(56, 56))
        _, preds, _, _ = model.predict(data[t:t+1])
        gt_skeleton = coordinates[t].astype(np.uint8)  
        pred_skeleton = preds[0].astype(np.uint8)
        img = data[t].astype(np.uint8)
...
def draw_images_separate(model, img_idxs: list[int]):  
    pathlib.Path("result").mkdir(parents=True, exist_ok=True)
    # GENERATE RESULT IMAGES
    filenames = []
    idx = 0
    ...
    for t in img_idxs:
        ...
        for i in range(14):
            cv2.circle(img, center=tuple(gt_skeleton[i][0:2]), radius=2, color=(0, 255, 0), thickness=2)
            cv2.circle(img, center=tuple(pred_skeleton[i][0:2]), radius=2, color=(255,0,0), thickness=2)
        # draw the lines
        for j in ((13, 12), (12, 8), (12, 9), (8, 7), (7, 6), (9, 10), (10, 11), (2, 3), (2, 1), (1, 0), (3, 4), (4, 5)):
            cv2.line(img, tuple(gt_skeleton[j[0]][0:2]), tuple(gt_skeleton[j[1]][0:2]), color=(0, 255, 0), thickness=1)
            cv2.line(img, tuple(pred_skeleton[j[0]][0:2]), tuple(pred_skeleton[j[1]][0:2]), color=(255,0,0), thickness=1)
        # solve the mid point of the hips
        cv2.line(img, tuple(gt_skeleton[12][0:2]), tuple(gt_skeleton[2][0:2] // 2 + gt_skeleton[3][0:2] // 2), color=(0, 255, 0), thickness=1)
        cv2.line(img, tuple(pred_skeleton[12][0:2]), tuple(pred_skeleton[2][0:2] // 2 + pred_skeleton[3][0:2] // 2), color=(255,0,0), thickness=1)
        cv2.putText(img, 'green = gt', (130,230), 1, 0.8, color=(0,255,0), thickness=1)
        cv2.putText(img, 'red = predicted', (130,245), 1, 0.8, color=(255,0,0), thickness=1)
        ...
        plt.imshow(img)
        plt.axis('off')
        plt.title(f'lsp_{t}', fontsize=40)

        idx += 1

    filename = "./result/lsp_joint.png"
    plt.savefig(filename, bbox_inches='tight')
    filenames.append(filename)
    return filenames